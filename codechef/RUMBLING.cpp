...
void calc()
{
    ...
    {
        if (s[i] == 'E') a[i] = 0;
        else if (s[i] == 'W') a[i] = min(2 * x, 2 * y);
        else if (s[i] == 'N') a[i] = min(x, 3 * y);
        else a[i] = min(y, 3 * x);
    }
    repp(i, n - 1) a[i] += a[i - 1];
    for (ll i = n - 1; i >= 0; i--)
    {
        if (s[i] == 'W') b[i] = 0;
        else if (s[i] == 'E') b[i] = min(2 * x, 2 * y);
        else if (s[i] == 'S') b[i] = min(x, 3 * y);
        else b[i] = min(y, 3 * x);
    }
    for (ll i = n - 2; i >= 0; i--) b[i] += b[i + 1];
    ll res = min(b[0], a[n - 1]);
    rep(i, n - 1) res = min(res, a[i] + b[i + 1]);
    cout << res;
}
